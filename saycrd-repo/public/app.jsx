...
<div style={{ position: "absolute", bottom: "32%", left: "50%", transform: "translateX(-50%)", fontSize: 10, letterSpacing: "0.35em", color: "rgba(127,212,255,0.9)", fontFamily: FB, textTransform: "uppercase", animation: "pulse 2s ease infinite" }}>Analyzing your words</div>
</div>
);
}
...
var seed = pi * 137.508;
pts.push({ x: ((seed * 7.3) % 100), y: ((seed * 3.1) % 100), size: 1 + (pi % 3) * 0.8, dur: 6 + (pi % 5) * 2.4, delay: -(pi % 7) * 1.3, opacity: 0.12 + (pi % 4) * 0.1, color: colors[pi % colors.length] });
}
return pts;
}, [colors]);
...
return (
<circle key={pi} cx={p.x + "%"} cy={p.y + "%"} r={p.size} fill={p.color} opacity={p.opacity}>
<animate attributeName="cy" values={p.y + "%;" + (p.y - 28) + "%;" + (p.y - 56) + "%"} dur={p.dur + "s"} begin={p.delay + "s"} repeatCount="indefinite" calcMode="linear"/>
<animate attributeName="opacity" values={p.opacity + ";" + Math.min(p.opacity * 2.5, 0.7) + ";0"} dur={p.dur + "s"} begin={p.delay + "s"} repeatCount="indefinite"/>
<animate attributeName="r" values={p.size + ";" + (p.size * 1.6) + ";0"} dur={p.dur + "s"} begin={p.delay + "s"} repeatCount="indefinite"/>
</circle>
);
})}
</svg>
...
<div style={{ position: "absolute", bottom: "4%", left: 0, right: 0, maxHeight: "48%", overflowY: stepKey === 2 ? "hidden" : "auto", overflowX: "hidden", scrollbarWidth: stepKey === 2 ? "none" : undefined, zIndex: 2, display: "flex", justifyContent: "center", alignItems: "flex-start" }}>{stage}</div>
</div>
);
}
...
<div style={{ fontSize: 18, fontFamily: FD, fontStyle: "italic", color: "rgba(255,107,107,0.9)", lineHeight: 1.4 }}>Error: {(err.length > 90 ? err.slice(0,90)+"…" : err)}</div>
...
var handlePD = function(e) { if(isMobile)return; if(e.target.closest("[data-slider]"))return; var r=ref.current?ref.current.getBoundingClientRect():null; if(!r)return; setDragOff({x:e.clientX-r.left,y:e.clientY-r.top}); };
...
{isMobile && <div style={{ fontSize: 11, color: "rgba(255,255,255,0.32)", fontFamily: FB, letterSpacing: "0.04em", marginTop: -6, marginBottom: 12 }}>tap any line below to react to it</div>}
...
try {
				...
				var top3Detail = _branches.slice(0,3).map(function(b){
					var match = themes.find(function(t){ return t.label === b.label; });
...
+ "RULES: Do NOT defend or reframe the original. Revise the section to incorporate their feedback. Remove or replace the sentence that didn't land. If they gave a note, use their words. Keep the same structure (2-3 short paragraphs). No poetry. Plain, clear language. Their truth overrides. If the feedback is \"Don't Understand\" and they gave a note about what's unclear, clarify or simplify that sentence so it's easier to grasp.\n\n"
	+ "GENDER — CRITICAL: The subject's gender is unknown unless they stated it in their own quoted words. Do not infer it from a name, an archetype, a topic, or the original report. Never use he, she, him, her, his, hers. Address them as \"you\"/\"your\", or \"they\"/\"their\"/\"the subject\". If the original used she/her or he/him, that was a mistake — rewrite it out.\n\n"
...
+ "VOICE: Use ONE consistent voice throughout — either \"you\" (second person) or \"they\"/\"the subject\" (third person). Do not mix. Never use he, she, him, her, his, hers. When quoting their words, use \"You said: \\\"...\\\"\" or \"They said: \\\"...\\\"\" depending on which voice you chose. Pick one and stick to it.\n\n"
	+ "GENDER — CRITICAL: The subject's gender is unknown unless they stated it in their own quoted words in THIS session's data. Do not infer it from a name, an archetype, a topic, a previous report, or anything else. Never use he, she, him, her, his, hers. Address them as \"you\"/\"your\", or \"they\"/\"their\"/\"the subject\" if third person. An archetype in SESSION ARC is a metaphor, not a gender. If a previous report used she/her or he/him, that was a mistake — do not copy it.\n\n"
...
try {
				...
				var wgTop3Labels = Object.keys(wgFreq).sort(function(a,b){ return wgFreq[b]-wgFreq[a]; }).slice(0,3);
				if (wgTop3Labels.length === 0) { setWgLineReady(true); return; }
				...
				var wgTop3Detail = wgTop3Labels.map(function(lbl) {
					var match = themes.find(function(t){ return t.label === lbl; });
					var extra = match ? (match.short_desc || match.why || "") : "";
...
function SAYCRDFlow() {
  ...
  // Handed over from landing.compiled.js: the visitor pressed "start a session"
  // on the standalone homepage, which loads THIS bundle rather than navigating
  // anywhere. Resume that intent through the same beginSessionOrGate() call the
  // landing's own onStart made before the split, so the disclaimer gate, free
  // guest sessions, credits and the paywall are still decided in exactly one
  // place. Nothing about entitlement is re-implemented on the landing side.
  //
  // useLayoutEffect, not useEffect: it runs after the first commit but BEFORE
  // the browser paints, so the homepage this bundle would otherwise draw at
  // phase 0 is never shown and then pulled away — the flash that #53 fixed.
  useLayoutEffect(function(){
    if (typeof window === "undefined" || !window.__SAYCRD_START_REQUESTED) return;
    // One-shot: a later remount (or a manual return to the homepage) must not
    // silently start another session.
    window.__SAYCRD_START_REQUESTED = false;
    beginSessionOrGate(function(){ setPhase(1); });
  }, []);
...
// Release the container first when the standalone homepage is what is currently
// on screen (landing.compiled.js mounted its own root into the same #root).
// React refuses to hand one container to createRoot() twice, and without this
// the handover throws instead of replacing the page. A no-op in the single-bundle
// build and for a returning visitor, where the landing bundle never loaded.
if (typeof window !== "undefined" && typeof window.__saycrdUnmountLanding === "function") {
  window.__saycrdUnmountLanding();
}
